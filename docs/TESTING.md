...
The API integration suite composes authenticated HTTP, PostgreSQL, viem signing/broadcast, deployed Anvil escrow/outcome/ERC-8004-compatible contracts, receipt/state attestation, provider assignment, submission, verification, reputation, and exact replay. It uses a deliberately labelled content-addressed evidence test adapter and exercises both complete branches: `PASSED -> SETTLING -> PAID -> reputation 100` and hard-failure `FAILED -> FAILED_FINAL -> REFUNDED -> reputation 0`. It verifies both outcome commitments on-chain, escrow terminal state, parsed/read-back feedback, one payment/refund row, one reputation event, duplicate-finalization rejection, exact replay, and removal of canonical operation/signed-transaction payloads after confirmation. This test does not substitute for or claim live 0G Storage or ERC-8004 network verification.
